    ...
    server.setMessageCallback([](const std::shared_ptr<TcpConnection>& conn, Buffer* buf) {
        ...
        if (eof != std::string::npos) {
            ...
            static const std::string body = [](){    
                std::string s;
                s.reserve(65536 * 4);
                for(size_t i = 0; i < 4096 * 4; ++i) s += "1234567890123456";
                return s;
            }();
...
//taskset -c 0,2,4,6,8,10,12,14 ./http_server
...
//sudo perf record -F 99 -p 1528 -g -- sleep 20
//sudo perf script | ./FlameGraph/stackcollapse-perf.pl | ./FlameGraph/flamegraph.pl > out.svg

//sudo perf trace -p 1528 --summary -- sleep 10

//sudo prlimit --pid 7342 --memlock=unlimited --nofile=1000000